...
{   
    ...
    try{
        c.openComunicators(
            cameraIndex,
            "appsrc is-live=true ! queue ! videoconvert ! video/x-raw, format=I420 ! queue ! vaapih264enc keyframe-period=30 quality-level=7 ! video/x-h264, stream-format=avc, profile=baseline ! queue ! rtph264pay config-interval=1 ! udpsink host={ip} port=5000",
            pi_credentials == NULL ? string() : pi_credentials,
            "udpsrc port=5000 ! application/x-rtp, media=video, clock-rate=90000, encoding-name=JPEG, payload=26, a-framerate=20 ! rtpjitterbuffer drop-on-latency=true latency=100 ! rtpjpegdepay ! queue ! decodebin ! videoconvert ! queue ! appsink"
        );
    }
    catch(...){
        return 0;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        Mat resized;
        cv::resize(output, resized, Size(640, (640.0/c.getScreenWidth())*c.getScreenHeight()));
	    ...
	    if(debugMode){
            ...
            putText(resized, fps_text, Point(30,30), 
                FONT_HERSHEY_COMPLEX_SMALL, 0.5, Scalar(0,250,0), 1);
        }	   

        imshow("saida", resized);
        char key = waitKey(1);
        if(key == 27)
    ...
    for(int i = 1; i < argc; i++){
        if(strcmp(argv[i], "-d") == 0){
            ...
            cout << "Minimal usage: ./oficial.out -d -cp <path to camera parameters> -m <path to model> -bp <path to board parameters>" << endl
                 ...
                 << "-bp <path>: [Required] Path of board parameters" << endl
                 << "-cp <path>: [Required] Path of camera calibration file" << endl
                 << "-pi <user@ip>: [Required if -d not passed] Informs user@ip of raspberry" << endl
                 << "-iphost <host_ip>: [Required if -d not passed] Informs ip of computer host" << endl;
            exit(-1);
        }
    }
